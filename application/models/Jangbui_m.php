...
class Jangbui_m extends CI_Model {
	...
	public function getlist($text1, $start, $limit)
	{
			...
			$sql = " select jangbu.*, product.name as product_name
					...
					where jangbu.io = 0 AND jangbu.writeday = '$text1' 
					order by jangbu.no limit $start, $limit ";
			 */
			$where = array("jangbu.io"=>0, "jangbu.writeday"=>$text1);
			return $this->db
			   ->limit($limit, $start)
	...
	{
		// $sql = " select * from jangbu where io=0 AND writeday='$text1' ";
		$where = array("io"=>0, "writeday"=>$text1);
		return $this->db
